   ...
   FROM rooms WHERE checkin <= '$checkin' AND checkout >='$checkout'))";
                    ...
                    if(mysqli_num_rows($check) > 0)
                    {
                        ...
                        if($send)
                        {
                            ...
                            $sql3="UPDATE room_category  SET available=available - 1 WHERE roomname='$roomcat'";
                            
                            $send2=mysqli_query($this->db,$sql3);
                            if($send2)
                            {
             ...
             public function edit_all_room($checkin, $checkout, $name, $phone, $book, $id)
            {
                                
                        $sql2="UPDATE rooms  SET checkin='$checkin', checkout='$checkout', name='$name', phone='$phone', book='$book' WHERE room_id=$id";
                         $send=mysqli_query($this->db,$sql2);
                        if($send)
                        {
                            $result="Your Room has been booked!!";
             ...
             public function edit_room_cat($roomname, $room_qnty, $available, $no_bed, $bedtype,$facility,$price,$room_cat)
            {
